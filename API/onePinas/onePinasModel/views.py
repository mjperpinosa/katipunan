...
def get_employee_tasks(request):
    ...
    tasks = Task.objects.filter(assignee=emp).values()
    tasks = list(tasks)
...
def create_project(request):
    ...
    sup = None
    try:
        sup = Profile.objects.get(id=profile_id)
...
def assign_task_to_employee(request):
    ...
    data = {"message": "successful", "first_name": assignee.first_name, "last_name": assignee.last_name}
...
def get_project_tasks(request):
    ...
    for task in tasks:
        ...
        if task.assignee is not None:
            pro = Profile.objects.get(id=task.assignee.id)
            name = str(pro.first_name) + " " + str(pro.last_name)
        assignee_data.append({"name": name})

    tasks = Task.objects.filter(project=project).values()

    emp_data = []
    employees = Account.objects.filter(creator=project.creator)
    for emp in employees:
        pro = Profile.objects.get(id=emp.profile.id)
        d = {"id": pro.id, "first_name": pro.first_name, "last_name": pro.last_name}
        emp_data.append(d)
    ...
    return JsonResponse({"tasks": tasks, "employees": emp_data, "assignee_data": assignee_data, "title": project.name, "description": project.description}, safe=False)
